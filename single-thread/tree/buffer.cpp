...
const uint16_t INNER_NODE_SIZE = 44;
...
struct INode { // inner node, allocated on DRAM 
    ...
    void insert_hotspot(_key_t key, _value_t val) {
        ...
        uint8_t cnt = (uint8_t)_mm_popcnt_u64(bitmap);
        if (cnt == BUFFER_SIZE) {
            if (turn_) {
                for (int i = 0; i < BUFFER_SIZE; ++i) {
                    if (finger_prints_[i] & 0x80) {
                        finger_prints_[i] &= 0x7F;
                    } else {
                        finger_prints_[i] = fp;
                        buffer_[i] = {key, (char *)val};
                    }
                }
            } else {
    ...
    char * pln_update_child(_key_t key) { // find the record whose key is the last one that is less equal to key
        char fp = finger_print2(key);
        for (int i = 0; i < BUFFER_SIZE; ++i) {
            if ( (bitmap & (1 << i)) && (finger_prints_[i] & 0x7f) == fp ) {
                bitmap &= ~(1 << i);
            }
        }
    ...
    char * pln_get_child(_key_t key, bool &hit) { // find the record whose key is the last one that is less equal to key
        ...
        uint64_t i = 0;
        for (; i < BUFFER_SIZE; ++i) {
            if ( (bitmap & (1 << i))  && (finger_prints_[i] & 0x7F) == fp && buffer_[i].key == key) {
                hit = true;
                if ( !(finger_prints_[i] & 0x80) ) {
                    finger_prints_[i] |= 0x80;
                } 
                return buffer_[i].val;
            }
        }
