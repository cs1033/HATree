...
const uint16_t BUFFER2_SIZE = 16;
...
class hatree2 {
    public:
        hatree2(string path, bool recover) {
            if(recover == false) {
                galc = new PMAllocator(path.c_str(), false, "hatree2");
                root_ = new INode(true);
                //entrance_ = (hatree2_entrance_t *) galc->get_root(sizeof(hatree2_entrance_t));
                //entrance_->root = galc->relative(root_);
                LNode * leaf = (LNode *)galc->get_root(sizeof(LNode));
                root_->leftmost_ptr_ = (char *)leaf;
            } else {
                galc = new PMAllocator(path.c_str(), true, "hatree2");
                LNode * leaf = (LNode *)galc->get_root(sizeof(LNode));
                root_->leftmost_ptr_ = (char *)leaf;
            }
        }
