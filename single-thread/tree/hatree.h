...
class hatree {
    public:
        hatree(string path, bool recover) {
            if(recover == false) {
                galc = new PMAllocator(path.c_str(), false, "hatree");
                root_ = new INode(true);
                //entrance_ = (hatree_entrance_t *) galc->get_root(sizeof(hatree_entrance_t));
                //entrance_->root = galc->relative(root_);
                LNode * leaf = (LNode *)galc->get_root(sizeof(LNode));
                root_->leftmost_ptr_ = (char *)leaf;
            } else {
                galc = new PMAllocator(path.c_str(), true, "hatree");
                LNode * leaf = (LNode *)galc->get_root(sizeof(LNode));
                root_->leftmost_ptr_ = (char *)leaf;
            }
        }
