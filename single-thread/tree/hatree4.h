...
const uint16_t BUFFER2_SIZE = 32;
...
struct INode { // inner node, allocated on DRAM 
    ...
    void insert_hotspot(_key_t key, _value_t val) {
        ...
        uint32_t newBitmap = bitmap;
        if (_mm_popcnt_u32(bitmap) == BUFFER_SIZE - start + BUFFER2_SIZE) { // cache is full
    ...
    char * pln_update_child(_key_t key, _value_t val) { // find the record whose key is the last one that is less equal to key
        ...
        for (int i = 0; i < BUFFER_SIZE + BUFFER2_SIZE; ++i) {
            ...
            //     bitmap &= ~(3ULL<<(2*i));
            // }
            if ( ((bitmap >> (2*i)) & 0x3) && finger_prints_[i] == fp && key == recs_[i + INNER_NODE_SIZE - BUFFER_SIZE].key ) {
...
class hatree4 {
    public:
        hatree4(string path, bool recover) {
            if(recover == false) {
                galc = new PMAllocator(path.c_str(), false, "hatree4");
                root_ = new INode(true);
                //entrance_ = (hatree4_entrance_t *) galc->get_root(sizeof(hatree4_entrance_t));
                //entrance_->root = galc->relative(root_);
                LNode * leaf = (LNode *)galc->get_root(sizeof(LNode));
                root_->leftmost_ptr_ = (char *)leaf;
            } else {
                galc = new PMAllocator(path.c_str(), true, "hatree4");
                LNode * leaf = (LNode *)galc->get_root(sizeof(LNode));
                root_->leftmost_ptr_ = (char *)leaf;
            }
        }
