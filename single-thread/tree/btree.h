...
const int NODE_SIZE = 14;                  // INode size 256B
const int INNER_UNDERFLOW_CARD = INNER_NODE_SIZE / 2;
const int LEAF_UNDERFLOW_CARD = NODE_SIZE / 3;  // Underflow limit
const int INNER_MAX = INNER_NODE_SIZE  ; // one quarter of the slots are reserved as read buffer
 
struct LNode;
struct INode;
  
struct btree_entrance_t{
    void * root;
};
...
class btree {
    public:
        btree(string path, bool recover) {
            if(recover == false) {
                ...
                galc = new PMAllocator(path.c_str(), true, "btree");
                root_ = new INode(true);
                LNode * leaf = (LNode *)galc->get_root(sizeof(LNode));
                root_->leftmost_ptr_ = (char *)leaf;
            }
        ...
        void statistics() {
            INode* cur = root_;
            std::vector<uint32_t> cnt(INNER_NODE_SIZE + 1, 0);
            uint64_t sum = 0, n = 0;

            while(!cur->is_parent_of_leaf_) { // no prefetch here
                char * child_ptr = cur->leftmost_ptr_;
                cur = (INode *)child_ptr;
            }
            while (cur) {
                cnt[cur->count_]++;
                n++;
                cur = (INode *)cur->sibling_ptr_;
            }

            uint64_t midSum = 0, mid = 0;
            for (int i = 0; i <= INNER_NODE_SIZE; ++i) {
                sum += cnt[i]*i;
            ...
            //ave
            std::cout << (0.0 + sum) / n / INNER_NODE_SIZE << "\t";
            //mid
            std::cout << (0.0 + mid) / INNER_NODE_SIZE<< std::endl;
        }
